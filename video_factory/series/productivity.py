"""
AI Productivity Explainer Video Producer
Produces 8-15 minute faceless explainer videos with TTS + slide visuals.
Ties to the AI Companion tool.
"""
...
def _load_templates():
    with open(TEMPLATES_DIR / "productivity_scripts.json") as f:
        return json.load(f)
    ...
    if PRODUCTION_LOG.exists():
        with open(PRODUCTION_LOG) as f:
            for entry in json.load(f):
                if entry.get("series") == "productivity":
...
def produce_productivity_video(config=None) -> dict:
    """Produce a complete AI productivity explainer video."""
    ...
    video_id = f"prod_{config['id']}_{timestamp}"
    ...
    try:
        ...
        generate_thumbnail(thumb_title, "AI Productivity Guide", thumb_path, style="explainer")
        ...
        send_production_complete(
            series="AI Productivity / Explainer",
            title=title,
        ...
        print(f"\n❌ FAILED: {e}")
        send_error(f"Productivity video failed: {title}\n\n{e}")
