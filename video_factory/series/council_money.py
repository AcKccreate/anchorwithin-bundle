"""
Global Council: Money & Finance Shorts Producer
Produces 30-60 second vertical debate videos — money/finance topics.
...
def _load_templates():
    with open(TEMPLATES_DIR / "council_money_scripts.json") as f:
        return json.load(f)
    ...
    if PRODUCTION_LOG.exists():
        with open(PRODUCTION_LOG) as f:
            for entry in json.load(f):
                if entry.get("series") == "council_money":
...
def produce_council_money_short(config=None) -> dict:
    """Produce a complete Global Council money/finance debate short video."""
    ...
    video_id = f"gcm_{config['id']}_{timestamp}"

    work_dir = OUTPUT_DIR / video_id
    work_dir.mkdir(parents=True, exist_ok=True)

    result = {
        "series": "council_money",
    ...
    try:
        print(f"\n{'='*60}")
        print(f"PRODUCING MONEY SHORT: {title}")
        print(f"{'='*60}")
        ...
        generate_thumbnail(topic, "The Global Council — Money",
        ...
        print(f"\n❌ FAILED: {e}")
        send_error(f"Council Money short failed: {title}\n\n{e}")
