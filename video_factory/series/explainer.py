"""
AI Money / Side Hustle Explainer Video Producer
...
def _load_templates():
    with open(TEMPLATES_DIR / "explainer_scripts.json") as f:
        return json.load(f)
...
def produce_explainer_video(script: dict = None) -> dict:
    """Produce a complete AI money/side hustle explainer video."""
    templates = _load_templates()
    config = script or _get_next_script(templates)
    ...
    video_id = f"exp_{config['id']}_{timestamp}"
    ...
    try:
        ...
        # Use first few words of title for thumbnail
        thumb_title = title.split("(")[0].strip() if "(" in title else title
        generate_thumbnail(thumb_title, "AI Money Guide", thumb_path, style="explainer")
        ...
        # Calculate frame duration based on audio length / number of slides
        ...
        print(f"\n❌ FAILED: {e}")
        send_error(f"Explainer video failed: {title}\n\n{e}")
