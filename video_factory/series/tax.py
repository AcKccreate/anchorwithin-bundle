"""
AI Tax Tips Explainer Video Producer
Produces 8-15 minute faceless explainer videos with TTS + slide visuals.
Ties to the AI Tax Companion tool.
IMPORTANT: No medical/healthcare content.
"""
...
def _load_templates():
    with open(TEMPLATES_DIR / "tax_scripts.json") as f:
        return json.load(f)
...
def produce_tax_video(config=None) -> dict:
    """Produce a complete AI tax tips explainer video."""
    ...
    video_id = f"tax_{config['id']}_{timestamp}"
    ...
    try:
        ...
        generate_thumbnail(thumb_title, "AI Tax Guide", thumb_path, style="explainer")
        ...
        print(f"\n❌ FAILED: {e}")
        send_error(f"Tax video failed: {title}\n\n{e}")
