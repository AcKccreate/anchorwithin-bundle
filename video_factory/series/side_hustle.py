"""
AI Side Hustle Explainer Video Producer
Produces 8-15 minute faceless explainer videos with TTS + slide visuals.
Ties to the AI Side Hustle Planner tool.
"""
...
def _load_templates():
    with open(TEMPLATES_DIR / "side_hustle_scripts.json") as f:
        return json.load(f)
    ...
    if PRODUCTION_LOG.exists():
        with open(PRODUCTION_LOG) as f:
            for entry in json.load(f):
                if entry.get("series") == "side_hustle":
...
def produce_side_hustle_video(config=None) -> dict:
    """Produce a complete AI side hustle explainer video."""
    ...
    video_id = f"hustle_{config['id']}_{timestamp}"
    ...
    try:
        ...
        generate_thumbnail(thumb_title, "AI Side Hustle Guide", thumb_path, style="explainer")
        ...
        send_production_complete(
            series="AI Side Hustle / Explainer",
            title=title,
        ...
        print(f"\n❌ FAILED: {e}")
        send_error(f"Side hustle video failed: {title}\n\n{e}")
