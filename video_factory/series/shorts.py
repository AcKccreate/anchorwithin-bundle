"""
Global Council Shorts Producer
Produces 30-60 second vertical debate videos with multiple characters.
"""
...
def _load_templates():
    with open(TEMPLATES_DIR / "shorts_scripts.json") as f:
        return json.load(f)
...
def produce_short(script: dict = None) -> dict:
    """Produce a complete Global Council short video."""
    ...
    video_id = f"gc_{config['id']}_{timestamp}"
    ...
    try:
        ...
        generate_thumbnail(topic, "The Global Council Debates",
        ...
        print(f"\n❌ FAILED: {e}")
        send_error(f"Global Council short failed: {title}\n\n{e}")
